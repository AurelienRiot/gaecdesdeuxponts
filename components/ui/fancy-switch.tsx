...
export interface FancySwitchProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "onChange"> {
  value?: OptionValue;
...
export const FancySwitch = React.forwardRef<HTMLDivElement, FancySwitchProps>(
  (
    ...
    const [activeIndex, setActiveIndex] = React.useState(() => {
      ...
      if (index === -1) {
        console.warn(`FancySwitch: No option found for value "${value}". Defaulting to first option.`);
        return 0;
      }
      return index;
    });
    ...
    React.useEffect(() => {
      const newIndex = memoizedOptions.findIndex((option) => option.value === value);
    ...
    const refCallback = (el: HTMLDivElement | null, index: number) => {
      if (el) {
